...
export class SubheaderService implements OnDestroy {
  ...
  breadcrumbsBtn: BehaviorSubject<BreadcrumbButtonModel[]> = new BehaviorSubject<BreadcrumbButtonModel[]>([]);
  descriptionSubject: BehaviorSubject<string> = new BehaviorSubject<string>('');
  descriptionStyledSubject: BehaviorSubject<string> = new BehaviorSubject<string>('');
  ...
  eventEmit: EventEmitter<string> = new EventEmitter();
  ...
  setBreadcrumbsBtn(breadcrumbBtn: BreadcrumbButtonModel[] = []) {
    this.breadcrumbsBtn.next(breadcrumbBtn);
  }
  ...
  setDescription(description: string) {
    ...
    this.descriptionSubject.next(data[0]);
    if (data[1]) {
      this.descriptionStyledSubject.next(data[1]);
    } else {
      this.descriptionStyledSubject.next(null);
    }
  }
  ...
  updateAfterRouteChanges(pathName) {
    ...
    if (aside && aside.title.trim() !== this.breadCrumbsSubject.getValue()[0]?.title.trim()) {
    } else {
      this.setBreadcrumbs([...this.breadCrumbsSubject.getValue()]);
    }
  }

  updateBreadcrumbs(breadcrumbs: BreadcrumbItemModel[], title?: string, breadcrumbBtn?: BreadcrumbButtonModel[], description?: string) {
    this.setBreadcrumbs(breadcrumbs);
    if (breadcrumbBtn?.length) this.setBreadcrumbsBtn(breadcrumbBtn);
    if (title) this.setTitle(title);
    if (description) this.setDescription(description);
  }

  updateSubBreadcrumbs(breadcrumbs: BreadcrumbItemModel[], title?: string, breadcrumbBtn?: BreadcrumbButtonModel[], description?: string) {
    this.setBreadcrumbs([...this.breadCrumbsSubject.getValue(), ...breadcrumbs]);
    this.setBreadcrumbsBtn(breadcrumbBtn);
    if (title) this.setTitle(title);
    if (description) this.setDescription(description);
  }
